import React from "react";

type HistoryProps = {
  history: {
    year: number;
    title: string;
    company: string;
  }[];
};

const WorkHistory: React.FC<HistoryProps> = ({ history }) => {
  return (
    <div>
      <div className="flex flex-col mx-6 items-start justify-start pt-[100px] min-h-screen relative ">
        <div className="text-[#CBB89D] my-6 tracking-widest">WORK HISTORY</div>
        {history.map((item, id) => (
          <div key={id} className=" text-start text-[#CBB89D] ">
            <div className="flex flex-row items-center gap-10 my-4">
              <div className="text-[26px]">{item.year}</div>
              <div className="flex flex-col">
                <div className="text-[26px] ">{item.title}</div>
                <div className="opacity-70">{item.company}</div>
              </div>
            </div>
            <div className="border w-screen -mx-6 border-[#806044] opacity-30"></div>
          </div>
        ))}
      </div>
    </div>
  );
};

export default WorkHistory;
